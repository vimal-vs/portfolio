      ...
      <table className='mt-12 w-full border-collapse text-left'>
        <thead className="sticky top-0 z-10 border-b border-slate-600/10 px-6 py-5 backdrop-blur">
          <tr>
            ...
            <th className="py-4 pr-8 text-sm font-semibold text-slate-200">Links</th>
          </tr>
        </thead>
        <tbody>
          {projects.map(project => {
            return (
              <tr className="border-b border-slate-300/10 last:border-none" key={project.id}>
                ...
                {project.link ? (<td className="text-sm py-4 align-top"><a href={project.link} className="hover:text-teal-300 hover:font-semibold" key={project.link}>Demo</a></td>) : <td></td>}
